...
def S_q(theta, a0=1, b0=0.01):
    ...
    alpha = tf.abs(theta[:, -1])  # the last column was logalpha todo
...
# sess.run(dw_data, feed_dict={Xs: X_b, ys: y_b, z: sample_z(mb_size_z, z_dim), lbd: lbd_0, alpha_power: alpha_1, lr_d: lr_d_1})
...
D_solver_n = tf.train.RMSPropOptimizer(learning_rate=lr_d).minimize(-Loss_n, var_list=theta_D_prior)
...
for it in range(n_iter):
    ...
    for _ in range(n_D):
        _, _, D_Loss_curr = sess.run([D_solver_a, D_solver_n, Loss_alpha],
                                     feed_dict={Xs: X_b, ys: y_b, z: sample_z(mb_size_z, z_dim), lbd: lbd_0,
    ...
    if it % 10 == 0:
        post, loss_curr = sess.run([G_sample, Loss_alpha], feed_dict={Xs: X_b, ys: y_b, z: sample_z(mb_size_z, z_dim), lbd: lbd_0, alpha_power: 1})
        print("mean:", np.mean(post), "std:", np.std(post))
