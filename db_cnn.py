...
Created on 8/29/18 8:36 PM

@author: Hanxi Sun

"""

# based on the db_nor_fix.py file
# only creates a reasonable cnn in generator
...
cnn_out_size = int(np.ceil(z_dim/mp_k) * fil_nb)  # output dim after the cnn layer


DIR = "/scratch/halstead/h/hu478/Stein_GAN" + "/output/temp/" if ON_SERVER else os.getcwd() + "/output/"
ID = now_str()
EXP = "db_nor_cnn_var={}_".format(ini_var) + "d_normal={}_".format(d_normal) + "lr={}_".format(lr_d_0)\
...
info.write(now_str() + '\n\n' + "Description: " + '\n\t' + "double cx with CNN in generator" +
           '\n\n' + ("=" * 80 + '\n') * 3 + '\n' +
           "Model Parameters: \n\t" +
...
def generator(z):
    ...
    G_h1 = lrelu(tf.matmul(G_c, G_W1) + G_b1, 0.1)
