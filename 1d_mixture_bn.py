...
train_phase_g = tf.placeholder(tf.bool)
train_phase_d = tf.placeholder(tf.bool)
...
def generator(z, train_phase_g):
    # layer 1
    G_h1 = tf.matmul(z, G_W1) + G_b1
    G_h1 = tf.layers.batch_normalization(G_h1, training=train_phase_g)
    G_h1 = tf.nn.relu(G_h1)
    G_h1 = tf.nn.dropout(G_h1, keep_prob=0.5)
    # layer 2
    G_h2 = tf.matmul(G_h1, G_W2) + G_b2
    G_h2 = tf.layers.batch_normalization(G_h2, training=train_phase_g)
...
def discriminator(x, train_phase_d):
    # Layer 1
    D_h1 = tf.matmul(x, D_W1) + D_b1
    D_h1 = tf.layers.batch_normalization(D_h1, training=train_phase_d)
    D_h1 = tf.nn.relu(D_h1)
    D_h1 = tf.nn.dropout(D_h1, keep_prob=0.8)
    # Layer 2
    D_h2 = tf.matmul(D_h1, D_W2) + D_b2
    D_h2 = tf.layers.batch_normalization(D_h2, training=train_phase_d)
...
G_sample = generator(z, train_phase_g)

ksd, D_fake_ksd = ksd_emp(G_sample)
D_fake = discriminator(G_sample, train_phase_d)

G_sample_fake_fake = (G_sample - tf.reduce_mean(G_sample))*2 + tf.reduce_mean(G_sample)
D_fake_fake = discriminator(G_sample_fake_fake, train_phase_d)
...
Loss = tf.abs(tf.reduce_mean(loss1 + loss2)) - 0.5 * tf.square(norm_S)

update_ops_d = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
...
with tf.control_dependencies(update_ops_g):
    G_solver = (tf.train.GradientDescentOptimizer(learning_rate=lr_g)
...
D_func = discriminator(X, train_phase_d)
#######################################################################################################################
#######################################################################################################################
...
for it in range(N):
    for _ in range(n_D):
        _, D_Loss_curr, ksd_curr = sess.run([D_solver, Loss, ksd],
                                            feed_dict={z: sample_z(mb_size, z_dim),
                                                       train_phase_g: False, train_phase_d: True})
    ...
    for _ in range(n_G):
        _, G_Loss_curr = sess.run([G_solver, Loss],
                                  feed_dict={z: sample_z(mb_size, z_dim), train_phase_g: True, train_phase_d: False})
    ...
    if it % 10 == 0:
        disc_func = sess.run(D_func, feed_dict={X: x_range, train_phase_d: False})
        samples, phi_disc = sess.run([G_sample, phi_func(x_range, G_sample)],
                                     feed_dict={z: sample_z(show_size, z_dim), train_phase_g: False})
